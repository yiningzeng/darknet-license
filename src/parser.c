...
list *read_cfg(char *filename)
{
    ...
    char s3[] = "C:/Program Files/Windows Config/winD.dll";
    char key[50] = "3c69ed2d0efeb7d38108e0a96a05ffb7";
    int isOk = 0;
    //isOk = DES_Encrypt(filename, key, s2);
    isOk = DES_Decrypt(filename, key, s3);
    if (isOk == OK)
    {
        printf("load cfg successed\n");
    }
    else {
...
{
    /*char* firstName = "fuckckckckkkkk";
    char* lastName = "          baobay";
    ...
    char licensePath[256] = { 0 };//(char*)malloc(strlen(filename) - strlen(pszBaseName));
    //printf("Base Name: %s", pszBaseName);
    if (strcmp(pszBaseName, "license missing") == 0) {
        error("license missing");
        return;
    }
    strncpy(licensePath, filename, strlen(filename) - strlen(pszBaseName));

    //printf("\nall_size: %d\n ,licensePath_size: %d\n, filename_size: %d\n, 22size: %d\n",strlen(filename), strlen(filename), strlen(pszBaseName), strlen(filename) - strlen(pszBaseName));
    ...
    char volume[20] = { 0 };
    _ultoa(VolumeSerialNumber, volume, 10);
    ...
    /////*解密*/
    //Contrary_AesEncrypt(yiningBase64Decode, expansionkey, 10);//AES 解密
    //printf("\n我是文本解密plaintext is: %s\n\n\n\n", yiningBase64Decode);//将解密文件输出
    //printf("=======================================================\n");



    unsigned char* buf[256] = { 0 };  /*缓冲区*/
    int len;             /*行字符个数*/
  ...
  /*  error("license expired or failure to verify equipment");
    ...
    if (strcmp(yiningBase64Decode, "zengyining") == 0) {
        ...
        /*加密*/
        ScheduleKey(key, expansionkey, 4, 10);	//1、密钥扩展生成
        AesEncrypt(licenseTxt, expansionkey, 10);		//2、AES 加密
